...
# This script plots routed flow (without calibration), routed flow (after calibration), together with USGS observed streamflow
...
# Load data
#========================================================
#--- Load routed flow data (without calibration) ---#
...
#--- Load routed flow data (after calibration) ---#
if cfg['INPUT_ROUTE_AFTER_CALI']['route_flow_format']=='RBM_formatted':  # if from formatted RBM output
    s_route_after_cali = my_functions.read_RMB_formatted_output(\
                            cfg['INPUT_ROUTE_AFTER_CALI']['route_path'], \
                            var='flow') / 1000 # convert to thousand cfs
elif cfg['INPUT_ROUTE_AFTER_CALI']['route_flow_format']=='Lohmann':  # if from formatted RBM output
    s_route_after_cali = my_functions.read_Lohmann_route_daily_output(\
                            cfg['INPUT_ROUTE_AFTER_CALI']['route_path'])\
                            / 1000 # convert to thousand cfs
else:
    print 'Error: unsupported routed flow format!'
...
if cfg['PLOT_OPTIONS']['plot_date_range']:  # if user-defined plotting date range
    ...
    data_avai_start_date, data_avai_end_date = my_functions.\
                        find_data_common_range([s_route, s_usgs, s_route_after_cali])
...
s_route_after_cali_to_plot = my_functions.select_time_range(s_route_after_cali, plot_start_date, plot_end_date)
...
fig = my_functions.plot_time_series(plot_date=True, \
            list_s_data=[s_usgs_to_plot, s_route_to_plot, s_route_after_cali_to_plot], \
            list_style=[cfg['PLOT_OPTIONS']['usgs_style'], \
                        cfg['PLOT_OPTIONS']['route_style'], \
                        cfg['PLOT_OPTIONS']['route_after_cali_style']],\
            list_label=[cfg['PLOT_OPTIONS']['usgs_label'], \
                        cfg['PLOT_OPTIONS']['route_label'], \
                        cfg['PLOT_OPTIONS']['route_after_cali_label']],\
