...
parser.add_argument("--hist_RBM_path", type=str, help="Historical RBM output path")
parser.add_argument("--future_RBM_paths_rcp45", type=str, help="eg. 'path_rcp45'")
parser.add_argument("--future_RBM_paths_rcp85", type=str, help="eg. 'path_rcp85'")
parser.add_argument("--future_GCM_names", type=str, help="eg. 'GCM1'")
...
future_scenario_basedir = cfg['INPUT']['future_scenario_basedir']
...
hist_RBM_path = args.hist_RBM_path # historical run. Lohmann routing output. year; month; day; flow(cfs)
future_RBM_paths_rcp45 = args.future_RBM_paths_rcp45 # future run. Lohmann routing output. year; month; day; flow(cfs); ['path1', 'path2']
future_RBM_paths_rcp85 = args.future_RBM_paths_rcp85 # future run. Lohmann routing output. year; month; day; flow(cfs); ['path1', 'path2']
future_GCM_names = args.future_GCM_names # future run. year; month; day; flow(cfs); ['GCM1', 'GCM2']

output_plot_basename = cfg['OUTPUT']['output_plot_basepath'] + '.' + usgs_site_code + '.' + future_GCM_names  # output plot path basename (suffix will be added to different plots)

#-------------------------------------------------
# list of RBM daily output data paths
hist_RBM_path = hist_RBM_path
control_RBM_path = future_RBM_paths_rcp45
list_future_RBM_path_rcp45 = [future_RBM_paths_rcp45, future_RBM_paths_rcp45, future_RBM_paths_rcp45]
list_future_RBM_path_rcp85 = [future_RBM_paths_rcp85, future_RBM_paths_rcp85, future_RBM_paths_rcp85]

nfuture = len(list_future_RBM_path_rcp45)
...
# Load data
#========================================================
hist_s_data = my_functions.read_RMB_formatted_output(hist_RBM_path)
control_s_data = my_functions.read_RMB_formatted_output(control_RBM_path)
...
for i in range(nfuture):
	future_s_data_rcp45 = my_functions.read_RMB_formatted_output(list_future_RBM_path_rcp45[i])
	list_future_s_data_rcp45.append(future_s_data_rcp45)
	future_s_data_rcp85 = my_functions.read_RMB_formatted_output(list_future_RBM_path_rcp85[i])
...
#
#fig = plt.savefig('%s.rcp45.future.Tstream.seas.png' %output_plot_basename, format='png', dpi=dpi)
#
## plot rcp85
...
#		add_info_text=True, model_info=model_info, stats='Monthly mean; historical - WY%d-%d; future projection - 1980s: WY1970-1999; \n          2020s: WY2010-2039; 2050s: WY2040-2069; 2080s: WY2070-2099' %(hist_plot_start_date.year+1, hist_plot_end_date.year)) \
#
#fig = plt.savefig('%s.rcp85.future.Tstream.seas.png' %output_plot_basename, format='png', dpi=dpi)
...
#	s_GCM_45 = my_functions.read_RMB_formatted_output(future_RBM_paths_rcp45.replace(future_GCM_names, list_individual_GCM[i]))
...
#			title='Average annual mean stream T, {}, {}'.format(usgs_site_name, usgs_site_code), \
...
#
#fig = plt.savefig('%s.future.Tstream.WY_mean_boxplot.hist_control.png' %output_plot_basename, format='png', dpi=dpi)
...
fig = plt.savefig('%s.future.Tstream.WY_mean_boxplot.png' %output_plot_basename, format='png', dpi=dpi)
...
#
#fig = plt.savefig('%s.future.Tstream_duration.png' %output_plot_basename, format='png', dpi=dpi)
