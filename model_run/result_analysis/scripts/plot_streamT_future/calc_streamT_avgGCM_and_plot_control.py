...
hist_s_data = my_functions.read_RMB_formatted_output(cfg['INPUT']['hist_rbm_path'], var='Tstream')
...
for i, GCM in enumerate(list_GCM):
    ...
    future_s_data_rcp45 = my_functions.read_RMB_formatted_output(\
                cfg['INPUT']['future_rbm_path'].replace('<GCM>', GCM)\
                                               .replace('<RCP>', 'rcp45'), \
                var='Tstream')
    future_s_data_rcp85 = my_functions.read_RMB_formatted_output(\
                cfg['INPUT']['future_rbm_path'].replace('<GCM>', GCM)\
                                               .replace('<RCP>', 'rcp85'), \
                var='Tstream')
...
fig = my_functions.plot_boxplot(list_data = [[hist_s_WY_mean]]+\
            ...
            title='Average annual mean stream temperature\n{}'\
...
fig = plt.savefig('{}.future.Tstream.WY_mean_boxplot.hist_control.png'\
