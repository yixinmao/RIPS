...
df_rbm = my_functions.convert_time_series_to_df(rbm_date, rbm_data[:,4:], ['Tstream', 'Thead', 'Tair'])  # convert to pd.DataFrame
s_rbm = df_rbm.ix[:,0]  # convert df to Series (only T_stream)
...
df_rbm_mon = my_functions.calc_monthly_data(df_rbm_to_plot)
# plot
fig = plt.figure(figsize=(12,8))
ax = plt.axes()
