...
def day_of_year(calendar_date):
    ''' Return day of year (treat Feb 29 as Feb 28)'''
    import datetime as dt
    if calendar_date.month==2 and calendar_date.day==29:
        day = 59
    else:
        day = (dt.datetime(2011, calendar_date.month, calendar_date.day) \
              - dt.datetime(2011,1,1)).days + 1
    return day
...
def calc_ts_stats_by_group(data, by, stat):
    ...
    elif by=='day_of_year':  # group by month and day
        if stat=='mean':
            data_result = data.groupby(lambda x:day_of_year(x)).mean()
