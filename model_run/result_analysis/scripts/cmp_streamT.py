...
usgs_data_path = '../data/USGS_data/T_' + usgs_site_code + '.txt'
usgs_data_col = 1

output_plot_basename = '../output/Tennessee_streamT_' + usgs_site_code # output plot path basename (suffix will be added to different plots)

#-------------------------------------------------

plot_start_date = dt.datetime(1975, 10, 1)  # start date shown on the plot (should be complete water years)
plot_end_date = dt.datetime(1981, 9, 30)  # end date shown on the plot
...
# USGS data
df_usgs = my_functions.read_USGS_data(usgs_data_path, columns=[3], names=['streamT'])  # [degC]
...
fig = plt.figure()
ax = plt.axes()
plt.plot_date(df_usgs_to_plot.index, df_usgs_to_plot.streamT, 'b-', label='USGS gauge')
plt.plot_date(df_rbm_to_plot.index, df_rbm_to_plot.streamT, 'r--', label='RBM')
plt.ylabel('Stream temperature (degC)', fontsize=16)
